...
public class InsertNoteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		response.setContentType("texthtml;charset=utf-8"); // 设置编码格式
		...
		String noteTime = request.getParameter("noteTime"); // 获取请 求参数

		NoteDao dao = new NoteDao(); // dao类

		Gson gson = new Gson(); // Gson解析类 ，用于将结果解析成JSON数据返回客户端
		Result result = new Result(); // 请 求结果类

		result.isSuccess = dao.insert(title, content, noteTime, tel); // 插入-条数据，并将结果保存到结果result对象中
		result.msg = result.isSuccess ? "记录成功" : "保存失败";

		response.getWriter().append(gson.toJson(result));// 将结 果解析成JSON数据通过响应对象，返回给客户端
	}
