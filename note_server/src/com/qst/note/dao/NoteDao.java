...
//向表note_table插入一条数据
	...
	public Boolean insert(String title, String content, String noteTime, String tel) {
		int id = new UserDao().getIDbyTel(tel); // 根据用户电话获取id
		...
		String nowTime = sdf.format(new Date(id)); // 获取当前时间
	...
	public NoteBean getNoteByID(int id) {
		...
		try {
			...
			PreparedStatement pst = (PreparedStatement) c.prepareStatement("select * from note_ _table where id=?");
			pst.setInt(1, id);
	...
	public boolean ModifyNote(int id, String title, String content, String noteTime) {
		...
		try {
			...
			PreparedStatement pst = (PreparedStatement) c
					.prepareStatement("update note_ table set tite=?.content=?.note time=?,update_ time=? where id=?");
			pst.setString(1, title);
